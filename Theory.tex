...
\titleformat{\chapter}[hang]{\normalfont \Huge \sffamily}{\thechapter. }{0 pt}{\Huge}
\pagestyle{plain}
...
\begin{itemize}
  ...
  \item Hence it is okay to increase the $\log_{10}(R+1)$ term
    ...
    (due to its use of libdivide,
    which only supports 32- and 64- bit integers)
    is $2^{63}-1$,
    and $\log_{10} 2^{63} < 20$,
    so we can safely substitute 20 for this term,
    regardless of what the actual $\Imax$ and $B$ used might be.
  \item We are assuming that our worst case is $a=5$,
    so $\log_{10} a > 0.698$.
...
(like if we use equation~\eqref{atan+10-239-515})
instead of $a=5$
...
not be assured of being able
to accurately compute \emph{any} digits
