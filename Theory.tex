...
\begin{document}
\chapter*{1. Identities equivalent to $\atan(1)$}
...
\end{equation}
This has the advantage of only requiring two passes, but has the
disadvantage that the series for $\frac{1}{5}$ converges more slowly
than those for $\frac{1}{10}$ and $\frac{1}{515}$ combined.
The proof of correctness may be obtained in a manner very similar
to the one shown above, and will not be spelled out here.
...
Richard Brent and Eugene Salamin (1976), which \emph{doubles} the number
...
arithmetic with ``digit''s of base \texttt{BASE} is how this code accomplishes
its task.

\chapter*{2. Error Analysis}
Let
\begin{equation}\label{function-f}
  f(x, n) = \frac{x^{2n+1}}{2n+1} \quad  [1 \le \tfrac{1}{x}; \text{ i.e., } 0 \le x \le 1]
\end{equation}
...
then the sum $\sum_{i=0}^\infty (-1)^i\, a_i$
converges; truncating this sum
after term $n$ will have an error whose magnitude
is at most $a_{n+1}$.
...
We will assume that $a=1/x$ is the worst-case choice for the relations being
considered. Substituting into \eqref{partial-f-a-n} gives
...
for some specified base $B$ and to-be-determined base-$B$-digit-count $D$:
\begin{equation}\label{target}
  \err(n) < B^{-D}
\end{equation}

Furthermore, we want to fit intermediate results into an integer which
cannot exceed some maximum \Imax.
...
It is acceptable to find a value $D'$ that is less than the actual maximum,
...
\begin{equation}\label{R-def}
  R = \frac{\Imax}{B+1}
\end{equation}
Combining substitutions of
\eqref{n-terms} and \eqref{R-def}
into equation~\eqref{target-inequality}
gives:
...
\end{equation}
Converting the digit count to base-10 digits involves multiplying
by $\log_{10} B$.
Since $(\log_B x)(\log_{10} B) = \frac{\log_B x}{\log_B 10} = \log_{10} x$,
...
\vbreak
To take a concrete example, let us assume that our worst-case is $a=5$
(as when evaluating the relation in equation~\eqref{atan+5-239}),
that $B=10^{12}$, and that $\Imax=2^{63}-1$,
...
$6\,446\,844$ decimal digits are allowed to be computed and printed,
to avoid the risk of errors from integer overflow
...
\begin{itemize}
  ...
  \item Hence it is okay to increase the $\log_{10}(R+1)$ term
    in equation~\eqref{d-constraint} by a little bit,
    as this would lower the bound on $d$ by a little bit.
    The largest $\Imax$ supported by the code
    (due to its use of libdivide, which only supports 32- and 64- bit
    integers)
    is $2^{63}-1$,
    and $\log_{10} 2^{63} < 20$, so we can safely
    substitute 20 for this term, regardless of what
    the actual $\Imax$ and $B$ used might be.
  \item We are assuming that our worst case is $a=5$, so $\log_{10} a > 0.698$.
  \item To minimize user confusion, we want to round the ``maximum''
    down to the nearest multiple of $\log_{10} B$, since we will always
    be outputting an integer number of base-$B$ ``digits''.
\end{itemize}
...
$\texttt{WORDDIGITS} = \log_{10} B$ (which is required to be an integer,
thus we end up with $\texttt{BASE} = B$ below)):
\begin{verbatim}
...
(like if we use
equation~\eqref{atan+10-239-515})
instead of $a=5$ (as with equation~\eqref{atan+5-239}),
then instead of using $\log_{10}5 \approx 698/1000$
...
$\log_{10} 1 = 0$ --- thus we would not be assured of
being able to accurately compute \emph{any} digits
of the result before encountering integer overflow
...
(the other reason is that even if we had infinite-precision
variables, the rate of convergence for a raw $\atan(1)$
is much too slow).
